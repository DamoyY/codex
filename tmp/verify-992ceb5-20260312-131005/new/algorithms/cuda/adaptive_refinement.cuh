...
__device__ inline Rgb sample_bilinear(const SampleGrid4x4 &samples, float x, float y)
{
    int ix = (int)floorf(x);
    int iy = (int)floorf(y);
    float fx = x - (float)ix;
    ...
    if (ix == -1 && iy == -1) { tl = samples.a; tr = samples.b; bl = samples.e; br = samples.f; }
    else if (ix == 0 && iy == -1) { tl = samples.b; tr = samples.c; bl = samples.f; br = samples.g; }
    else if (ix == 1 && iy == -1) { tl = samples.c; tr = samples.d; bl = samples.g; br = samples.h; }
    else if (ix == -1 && iy == 0) { tl = samples.e; tr = samples.f; bl = samples.i; br = samples.j; }
    else if (ix == 0 && iy == 0) { tl = samples.f; tr = samples.g; bl = samples.j; br = samples.k; }
    else if (ix == 1 && iy == 0) { tl = samples.g; tr = samples.h; bl = samples.k; br = samples.l; }
    else if (ix == -1 && iy == 1) { tl = samples.i; tr = samples.j; bl = samples.m; br = samples.n; }
    else if (ix == 0 && iy == 1) { tl = samples.j; tr = samples.k; bl = samples.n; br = samples.o; }
    else if (ix == 1 && iy == 1) { tl = samples.k; tr = samples.l; bl = samples.o; br = samples.p; }
    else { tl = samples.f; tr = samples.g; bl = samples.j; br = samples.k; } // fallback
    
    return bilinear_patch(tl, tr, bl, br, fx, fy);
}
...
__device__ inline Rgb refine_upscale(
    ...
    float scale_y)
{
    Rgb north = sample_bilinear(samples, pp_x, pp_y - scale_y);
    Rgb west  = sample_bilinear(samples, pp_x - scale_x, pp_y);
    Rgb east  = sample_bilinear(samples, pp_x + scale_x, pp_y);
    Rgb south = sample_bilinear(samples, pp_x, pp_y + scale_y);
