...
mod tests {
    ...
    fn esc_in_notes_mode_interrupts() {
        ...
        assert_eq!(answer.answers, vec!["Option 1".to_string()]);
        assert!(
            !response.answers.contains_key(INTERRUPTED_ANSWER_ID),
            "unexpected interrupt marker"
        );
    ...
    fn freeform_commit_resets_when_draft_changes() {
        ...
        let answer = response.answers.get("q1").expect("answer missing");
        assert_eq!(answer.answers, vec!["user_note: Committed".to_string()]);
        assert!(
            !response.answers.contains_key("q2"),
            "did not expect q2 answer yet"
        );
